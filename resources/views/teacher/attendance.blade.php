...
<div class="glass-card">
    <div class="card-header">
        <span>
            <i class="fas fa-calendar-check me-2" style="color:var(--cyan)"></i>
            {{ __('attendance.mark') }}
        </span>
        <a href="{{ route('teacher.dashboard') }}" class="btn-outline-custom" style="font-size:.83rem">
            <i class="fas fa-arrow-left me-1"></i> {{ __('common.back') }}
        </a>
    </div>

    @if(isset($noProfile) || $classes->isEmpty())
    <div style="text-align:center;padding:60px;color:var(--muted)">
        <i class="fas fa-chalkboard fa-3x mb-3" style="display:block;opacity:.25"></i>
        <p style="font-size:.9rem">{{ __('common.no_classes_assigned') }}</p>
    </div>
    @else
    ...
    {{-- Status Bar --}}
    <div id="statusBar" style="display:none;padding:12px 24px;background:rgba(6,182,212,.05);border-bottom:1px solid var(--border)">
...
.att-row:hover { border-color:rgba(6,182,212,.25); }
